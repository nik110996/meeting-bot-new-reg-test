...
public class SundayPartTwoWhoOtherState extends BaseChatState {

    @Override
    public void onStart() {
        var yes = ChatWork.getButton(ResBundle.getMessage("sundayPartTwoWhoOtherState.yes"), C_ACCEPT);
        var no = ChatWork.getButton(ResBundle.getMessage("sundayPartTwoWhoOtherState.no"), C_DECLINE);
        InlineKeyboardMarkup keyboard = InlineKeyboardMarkup.builder()
                .keyboardRow(List.of(yes, no))
                .build();

        Message send = ChatWork.sendMessage(chat.getUserId(), ResBundle.getMessage("sundayPartTwoWhoOtherState.ask"), ParseMode.MARKDOWNV2, keyboard);
    ...
    public void callbackQuery(String data) {
        switch (data) {
            case C_ACCEPT:
                Message accept = ChatWork.sendMessage(chat.getUserId(), ResBundle.getMessage("sundayPartTwoWhoOtherState.response"), ParseMode.MARKDOWNV2, true, true);
                chat.setBotMessageId(accept.getMessageId());

                /* добавить в meeting "согласился участвовать во встречах" */
                new MeetingDAO().create(new MeetingModel(chat.getUserId(), MeetingState.S_ACCEPT_MEETING.id(), null));

                ChatWork.changeChatState(chat, ChatState.MAIN);
                break;

            case C_DECLINE:
                Message decline = ChatWork.sendMessage(chat.getUserId(), ResBundle.getMessage("sundayPartTwoWhoOtherState.skip"), ParseMode.MARKDOWNV2, true, true);
